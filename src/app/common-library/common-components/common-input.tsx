...
import { DateRangePickerField } from '../forms/date-range-picker';
...
import { stringNomalize } from '../helpers/common-function';
...
const DefaultPlaceholder = {
  ...
  'date-time-range': 'COMMON_COMPONENT.INFINITE_SELECT.PLACEHOLDER',
...
export type InputRadioType = {
  ...
  placeholder?: string;
  options:
    | { value: any; label: string | ((...props: any) => ReactElement) }[]
    | ((...props: any) => { value: any; label: string }[]);
  [X: string]: any;
};
...
export const InputStringNumber = ({
  label,
  required,
  placeholder,
  className,
...
export const InputImage = ({ label, className, value, ...props }: InputImageType) => {
  return (
    <div className={className}>
      <CustomImageUpload value={value} {...props} label={label} />
    </div>
  );
};
...
export const InputFile = ({ label, className, value, ...props }: InputFileType) => {
  return (
    <div className={className}>
      <UploadFile value={value} {...props} label={label} />
    </div>
  );
};
export const InputTag = ({
  ...
  const [dataValue, setData] = useState(tagData ?? []);
  const loadData = async () => {
    ...
    try {
      ...
      const list = [...new Set(entities.data.result)];
      setData(list);
    } catch (err) {}
  };
...
export const InputSearchSelect = ({
  ...
  const loadOptions = useCallback(
    async (search: string, prevOptions: any, { page }: any) => {
      ...
      const entities = await onSearch(code);
      const count = entities.data?.paging?.total;
      const data = search
        ? [...new Set(entities.data)].filter((el: any) =>
            stringNomalize(el[keyField as string].toLowerCase()).includes(
              stringNomalize(search.toLowerCase()),
            ),
          )
        : [...new Set(entities.data)];
...
export const InputCheckBox = ({ label, optionData, ...props }: InputCheckBoxType) => {
  return <CheckBoxField {...props} optionData={optionData} label={label} />;
};
