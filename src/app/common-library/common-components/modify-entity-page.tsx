      ...
      <div
        ...
        {pl &&
          Object.values(pl).map((inputGroup, index) => {
            ...
            // console.log('_styleName', _styleName);
            return (
              <div
                key={`modify-entity-page${index}`}
                className={_className ?? 'col-12'}
                style={_styleName}>
                {_subTitle && _subTitle !== '' && (
                  <div
                    className="modify-subtitle text-primary"
                    style={{
                      fontWeight: 600,
                      paddingBottom: '30px',
                      marginBottom: 0,
                    }}>
                    {intl.formatMessage({ id: _subTitle }).toUpperCase()}
                  </div>
                )}
                <RenderForm
                  inputs={inputs}
          ...
          default: {
            ...
            return _className ? (
              <span key={`render_form_span${name}`} className={_className}>
                ...
                <RenderForm
                  inputs={innt}
                  inputClassName={_inputClassName ?? inputClassName}
                  prevKey={name}
                  mode={mode}
                />
              </span>
