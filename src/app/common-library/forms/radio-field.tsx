...
export function RadioField({
  ...
  const [fieldChecked, setFieldChecked] = useState(null);
      ...
      <div className={mode === 'horizontal' ? 'row' : ''}>
        ...
        <div className={mode === 'horizontal' ? GetClassName(labelWidth, false) : ''}>
          <RadioGroup
            ...
            onChange={e => {
              setFieldChecked(_innerValue);
              console.log({ e, name, _innerValue, values });
            ...
            <div className={'row no-gutters'}>
              {_innerOptions.map((val, index, arr) => {
                ...
                //   val: val.value,
                //   field: field.value,
                //   checked: val.value == field.value,
                // });
                return (
                  <FormControlLabel
                    className={optionsClassName}
                    value={val.value}
                    checked={val.value.toString() == field.value}
                    control={<StyledRadio />}
                    key={`radio_${name}_${index}_${JSON.stringify(val.value)}`}
                    label={
                      _.isString(val.label)
                        ? intl.formatMessage({ id: val.label })
                        : val.label({
                            setFieldValue,
                            handleChange,
                            values,
